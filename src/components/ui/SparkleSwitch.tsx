...
interface SparkleSwitchProps {
  checked: boolean;
  onChange: (checked: boolean) => void;
  disabled?: boolean;
  className?: string;
  previewOnly?: boolean; // 新增预览模式支持
}

export default function SparkleSwitch({
  checked,
  onChange,
  disabled = false,
  className = '',
  ...
  const inputId = previewOnly ? "preview-sparkle-toggle" : "sparkle-toggle";

  return (
    <div className={`toggle-cont ${disabled ? 'opacity-50 cursor-not-allowed' : ''} ${className}`}>
      <input
      ...
      <style jsx>{`
        ...
        .toggle-cont .toggle-label {
          ...
          padding: 0.35rem; /* 从0.5rem缩小到0.35rem */
        ...
        .toggle-label::before {
          ...
          width: calc(100% + 1rem); /* 从1.5rem缩小到1rem */
          height: calc(100% + 1rem); /* 从1.5rem缩小到1rem */
